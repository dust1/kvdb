...
impl KVParser {
    ...
    pub fn parser(&self) -> Result<KVStatement> {
        match &self.statements[0] {
            Statement::CreateTable { name, columns, .. } => Ok(KVStatement::CreateTable {
                name: translate_object_name(name),
                columns: columns.iter().map(translate_column_def).collect::<Vec<_>>(),
            }),
            ...
            Statement::Query(query) => Ok(KVStatement::Query(Box::new(translate_query(query)))),
            Statement::Insert { table_name, columns, source, .. } => Ok(KVStatement::Insert {
